...
# Two task names collide with paths that exist here, and the catch-all below is a pattern
# rule with a real prerequisite -- so make compares mtimes instead of running the task, and
# prints "is up to date" the moment the path is newer than `$(UVX)`. Verified: `touch book`
# is enough to make `make book` silently build nothing.
#
#   book     the `book/` directory holding the Marimo notebooks
#   license  the `LICENSE` file, on a case-insensitive filesystem (macOS)
#
# The header above says `.PHONY` cannot name unknown targets and that no file shares a
# task's name -- true of the generated shim in general, not of this repository.
#
# `.PHONY` alone is not the fix and makes it worse: it stops make applying the
# match-anything rule at all, so `make book` reports "Nothing to be done for `book'" and
# still runs nothing. The target needs `.PHONY` *and* an explicit recipe -- an explicit rule
# also beats the pattern rule, so this is what actually dispatches.
.PHONY: book license
book license: $(UVX)
	@$(UVX) $(RHIZA_TASK) $@
